...
//  - !fight @user via Streamerbot → API → Overlay
...
var WS_HOST  = (window.CC && CC.validate) ? CC.validate.sanitize(params.get('host') || '192.168.178.34', 'host') : (params.get('host') || '192.168.178.34');
var WS_PORT  = parseInt(params.get('port') || '9091');
var API_HOST = params.get('apihost') || WS_HOST;
var API_PORT = parseInt(params.get('apiport') || String(WS_PORT));
var TEST_MODE  = params.get('test') === '1';
var FORCE_LIVE = params.get('forcelive') === '1';

var COOLDOWN_MS      = 15000;   // 15s pro Angreifer
...
// ── API WebSocket ─────────────────────────────────────────
function connect() {
  if (TEST_MODE && !params.get('host')) return;
...
function handleMsg(msg) {
  // Spielstatus vom Admin
  if (msg.event === 'sf_game_status') {
    gameActive = !!msg.active;
    return;
  }

  // Fight-Command (vom Streamerbot via API broadcastAll)
...
function runFight(aName, dName) {
  ...
  showFight(aName, dName, shipA, shipD, rounds, winner, loser, function() {
    // Ergebnis an API senden (WS + HTTP fallback)
